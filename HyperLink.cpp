...
{
	m_crLinkColour      = RGB(0, 128, 0);     // Blue
	m_crVisitedColour   = RGB(128, 128, 64);   // Purple
...
void CHyperLink::SetMessageMode(HWND hWnd,UINT message)
{
	m_hMsgWnd = hWnd;
	m_nMsg = message;
}
...
void CHyperLink::OnClicked()
{
	if(m_nMsg > 0)
	{
		::PostMessage(m_hMsgWnd,m_nMsg,0,0);
		return;
	}
...
void CHyperLink::ReportError(int nError)
{
	...
	TDX_MessageBox(NULL,str, MB_ICONEXCLAMATION | MB_OK);
}
