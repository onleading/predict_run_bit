// KQSDlg.cpp : implementation file
...
CKQSDlg::CKQSDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CKQSDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CKQSDlg)
	//}}AFX_DATA_INIT
...
BOOL CKQSDlg::Create(CWnd *pParent)
{
	return CDialog::Create(CKQSDlg::IDD,pParent);
}

void CKQSDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CKQSDlg)
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CKQSDlg, CDialog)
	//{{AFX_MSG_MAP(CKQSDlg)
	ON_WM_PAINT()
	ON_WM_ERASEBKGND()
...
void CKQSDlg::DrawIt(CRect rcIn,CDC *pDC)
{
...
void CKQSDlg::OnOK() //不允许用户按OK
{
}

void CKQSDlg::OnCancel() 
{
	if(m_bLevinModal)
	{
...
//不擦除背景
BOOL CKQSDlg::OnEraseBkgnd(CDC* pDC) 
{
	return TRUE;
}

void CKQSDlg::OnSHQZ(UINT nID)
{
	if(m_bLevinModal)
	{
		TDX_MessageBox(m_hWnd,"请先关闭闪电下单对话框",MB_ICONINFORMATION);
	...
	if(m_nSel<0 || m_nSel>=MAX_USE_SHQZ_NUM)
		return;
	MemStkInfo *pInfo = g_pStockIO->Get_MemStkInfo(m_strCode[m_nSel],SH);
	if(!pInfo) return;
	if(!OnLine || !CanLevel2())
	{
		TDX_MessageBox(m_hWnd,"只有使用Level2行情主站才能使用[权证盘口王]功能!",MB_ICONINFORMATION|MB_OK);
		return;
	}

	if(g_GH.bKQS_RelationGP)
...
void CKQSDlg::OnKQSACK(WPARAM wParam,LPARAM lParam)
{
	if(m_nSel<0) return;
...
void CKQSDlg::DoLevin(int which,BOOL bIsBuy)
{
	if(m_nSel<0 || m_nSel>=MAX_USE_SHQZ_NUM)
		return;
	MemStkInfo *pInfo = g_pStockIO->Get_MemStkInfo(m_strCode[m_nSel],SH);
...
void CKQSDlg::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CKQSDlg::OnMouseMove(UINT nFlags, CPoint point) 
{
	int k,i=-1,j=-1,m=-1,n=-1;
	for(k=0;k<5;k++)
	{
