...
BOOL l_bDrawSeatName = FALSE;

USeat::USeat(LPCOMINFO pComInfo):UBase(pComInfo)
{
	m_nSel = 0;
	...
	memset(m_SeatRect,0,2*MAX_SEATNUM*sizeof(RECT));
...
int  USeat::ProcessMsg(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
		{
			...
			for(int i=0;i<2;i++)
			{
				for(int j=0;j<MAX_SEATNUM;j++)
				{
					if(RectPtIn(m_SeatRect[i][j],pt))
					{
						char SeatStrS[61]={0},SeatStrL[61]={0};
						long nSeatID = atol(m_Seat[i][j].SeatNo);
						GetSeatNameFromSID(nSeatID,SeatStrS,SeatStrL);
						CString tmpInfo="";
						tmpInfo.Format("%s\r\n%s\r\n%s\r\n双击打开经纪席位追踪",m_Seat[i][j].SeatNo,SeatStrS,SeatStrL);
						CRect tmpRc=m_SeatRect[i][j];
						tmpRc.OffsetRect(left,top);
						m_ToolTip.AddToolTip(m_pView,_F(tmpInfo),&tmpRc,TOOLTIP_BTURN);
					}
				}
			}
		}
	...
	case WM_LBUTTONDOWN:
		{
			...
			{
				l_bDrawSeatName = !l_bDrawSeatName;
				ProcessMsg(WM_PAINT);
			}
		}
		return 1;
			...
			for(int i=0;i<2;i++)
			{
				for(int j=0;j<MAX_SEATNUM;j++)
				{
					if(RectPtIn(m_SeatRect[i][j],pt))
					{
						char SeatStrS[61]={0},SeatStrL[61]={0};
						long nSeatID = atol(m_Seat[i][j].SeatNo);
						AfxGetMainWnd()->PostMessage(UM_SEARCH_HKSEAT,nSeatID,0);
					}
				}
			}
		}
...
void USeat::ReadData(BOOL bDataReady,BOOL bReqData)
{
	memset(&m_Hq,0,sizeof(CURRDATA));
	memset(&m_HqEx,0,sizeof(EXCURRDATA));
	if(OnLine)
...
void USeat::DrawIt(CDC *pDC)
{
	BOOL bDrawName = l_bDrawSeatName;
	...
	if(g_bTickTStatus)
		g_d.DrawLine(pDC, 0, 0, m_rcIn.Width(), 0);
	...
	g_d.DisplayText(pDC, rcTitle, fcolor, DT_LEFT|DT_VCENTER, _F("买一价: %.3f"), m_Hq.Buyp[0]);
	fcolor = VipColor.LEVELCOLOR;
	if (m_Hq.Sellp[0] > m_Hq.Close+0.00001)
		fcolor = VipColor.KPCOLOR;
	else if (m_Hq.Sellp[0] < m_Hq.Close-0.00001) 
		fcolor = VipColor.DOWNCOLOR;
	g_d.DisplayText(pDC, rcTitle, fcolor, DT_RIGHT|DT_VCENTER, _F("卖一价: %.3f"), m_Hq.Sellp[0]);
	//
	...
	g_d.Bar(pDC, &m_rcCon, BACK_BRUSH);
	g_d.DrawRect(pDC, &m_rcCon, VipColor.AXISCOLOR);
	m_rcCon.top+=3;
	...
	for(i=0;i < ColNum;i++)
	{
		...
		for(j=0;j < RowNum;j++)
		{
			...
			if(m_Seat[0][DrawNum].SeatNo[0]=='+' || m_Seat[0][DrawNum].SeatNo[0]=='-' || m_Seat[0][DrawNum].SeatNo[0]==' ' || m_Seat[0][DrawNum].SeatNo[0]==0)
			...
			else
			{
				...
				if(g_nLastQuerySeatID==atol(m_Seat[0][DrawNum].SeatNo))
					g_d.DisplayText(pDC,XFrom,YFrom+j*TextH+1,VipColor.VOLCOLOR,_F(SeatStr));
				else
					g_d.DisplayText(pDC,XFrom,YFrom+j*TextH+1,VipColor.TXTCOLOR,_F(SeatStr));
				m_SeatRect[0][DrawNum].left=XFrom;
				m_SeatRect[0][DrawNum].right=XFrom+Cell_Wid;
				m_SeatRect[0][DrawNum].top=YFrom+j*TextH+1;
				m_SeatRect[0][DrawNum].bottom=YFrom+j*TextH+TextH;
			}
			if(pOldFont) 
	...
	for(i=0;i < ColNum;i++)
	{
		...
		for(j=0;j < RowNum;j++)
		{
			if(DrawNum>=MAX_SEATNUM) 
				break;
			CFont *pOldFont = g_d.SelectFont(pDC,INFO_FONT);
			if(m_Seat[1][DrawNum].SeatNo[0]=='+' || m_Seat[1][DrawNum].SeatNo[0]=='-' || m_Seat[1][DrawNum].SeatNo[0]==' ' || m_Seat[1][DrawNum].SeatNo[0]==0)
			...
			else
			{
				...
				if(g_nLastQuerySeatID==atol(m_Seat[1][DrawNum].SeatNo))
					g_d.DisplayText(pDC,XFrom,YFrom+j*TextH+1,VipColor.VOLCOLOR,_F(SeatStr));
				else
					g_d.DisplayText(pDC,XFrom,YFrom+j*TextH+1,VipColor.TXTCOLOR,_F(SeatStr));
				m_SeatRect[1][DrawNum].left=XFrom;
				m_SeatRect[1][DrawNum].right=XFrom+Cell_Wid;
				m_SeatRect[1][DrawNum].top=YFrom+j*TextH+1;
				m_SeatRect[1][DrawNum].bottom=YFrom+j*TextH+TextH;
			}
			if(pOldFont) 
				g_d.RestoreFont(pDC,pOldFont); 			
			DrawNum++;
