...
UWTRank::UWTRank(LPCOMINFO pComInfo):UBase(pComInfo)
{
	m_bDataReady = FALSE;
	m_bEmpty = TRUE;
	m_nZoomFlag = 0;
...
int  UWTRank::ProcessMsg(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		{
			...
			if(i>=0)
				tmpInfo=g_pStockIO->Get_MemStkInfo(g_Mtl.WTRankings[i].Code,m_setcode);
			else if(j>=0)
				tmpInfo=g_pStockIO->Get_MemStkInfo(g_Mtl.WTRankings[10+j].Code,m_setcode);
			if(tmpInfo)
...
void UWTRank::ReadData(BOOL bDataReady,BOOL bReqData)
{
	if(!OnLine || !CanLevel2())
		return;
	if(!bDataReady)
	{
		if(bReqData)
		{
			...
			g_pGetData->GetWTRank(m_setcode,m_fltype);
		}
	}
	else
		g_pGetData->GetWTRankACK(g_Mtl.WTRankings);
}

void UWTRank::DrawPart1(CDC *pDC,int TotalHeight,int TextH,int XWidth,int XFrom[],int YFrom)
{
	if(TotalHeight<2*TextH) return;
	g_d.DisplayText(pDC,XFrom[0],YFrom,VipColor.TXTCOLOR,_F("委卖排名"));
	g_d.DisplayText(pDC,XFrom[1],YFrom,VipColor.TXTCOLOR,XWidth,_F("委托量"));
	g_d.DisplayText(pDC,XFrom[2],YFrom,VipColor.TXTCOLOR,XWidth,_F("成交量"));	
	YFrom+=TextH;
	...
	for(i=0;i<min(10,row);i++)
	{
		...
		if(tmpInfo)
		{
			...
			g_d.DisplayText(pDC,XFrom[1],YFrom,VipColor.VOLCOLOR,XWidth,"%.0f",g_Mtl.WTRankings[i].TotalOrderQty/tmpInfo->Unit);
			g_d.DisplayText(pDC,XFrom[2],YFrom,VipColor.VOLCOLOR,XWidth,"%.0f",g_Mtl.WTRankings[i].TotalVolumeTraded/tmpInfo->Unit);
			YFrom+=TextH;
		}		
	}
}

void UWTRank::DrawPart2(CDC *pDC,int TotalHeight,int TextH,int XWidth,int XFrom[],int YFrom)
{
	if(TotalHeight<2*TextH) return;
	g_d.DisplayText(pDC,XFrom[0],YFrom,VipColor.TXTCOLOR,_F("委买排名"));
	g_d.DisplayText(pDC,XFrom[1],YFrom,VipColor.TXTCOLOR,XWidth,_F("委托量"));
	g_d.DisplayText(pDC,XFrom[2],YFrom,VipColor.TXTCOLOR,XWidth,_F("成交量"));
	YFrom+=TextH;
	g_d.DotLine(pDC,0,YFrom-1,m_rcIn.right,YFrom-1,VipColor.AXISCOLOR,2);
	...
	for(i=0;i<min(10,row);i++)
	{
		tmpInfo=g_pStockIO->Get_MemStkInfo(g_Mtl.WTRankings[10+i].Code,m_setcode);
		if(tmpInfo)
		{
			size=g_d.DisplaySameWidtchText(pDC,XFrom[0],YFrom,GetBlockGPColor(tmpInfo,GridColor.Grid_CodeName_Color),tmpInfo->Name);
			m_BuyRect[i].SetRect(XFrom[0],YFrom,XFrom[0]+size.cx,YFrom+TextH-2);
			g_d.DisplayText(pDC,XFrom[1],YFrom,VipColor.VOLCOLOR,XWidth,"%.0f",g_Mtl.WTRankings[10+i].TotalOrderQty/tmpInfo->Unit);
			g_d.DisplayText(pDC,XFrom[2],YFrom,VipColor.VOLCOLOR,XWidth,"%.0f",g_Mtl.WTRankings[10+i].TotalVolumeTraded/tmpInfo->Unit);
			YFrom+=TextH;
		}		
	}
}

void UWTRank::DrawIt(CDC *pDC)
{
