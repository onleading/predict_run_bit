...
BEGIN_MESSAGE_MAP(CComView, CUnitView)
	...
	ON_COMMAND_RANGE(ID_CORNER_1,ID_CORNER_9,OnCornerClick)
	...
	ON_MESSAGE(UM_SWITCHTOPVIEW,OnSwitchTopView)
	...
	ON_COMMAND(ID_OPENURL,OnSwitchWeb)
...
int CComView::GetCurSwitch(DWORD AssisPad)
{
	switch(AssisPad)
	{
	case HQTICK_PAD:		return 0;
	case HQFJB_PAD:			return 1;
	case HQTICK2_PAD:		return 2;
	case HQMMP_PAD:			return 3;
	case HQZST_PAD:			return 4-2;
	case HQZS_PAD:			return 5-2;
	case HQCDP_PAD:			return 6-2;
	case CB_PAD:
	case BIGVOL_PAD:		return 7-2;
	case WEB_PAD:			return 8-2;	

	case HQ10QUEUE_PAD:		return 0;
	case HQ10WTFB_PAD:		return 1;
	case HQ10TICKSTAT_PAD:	return 2;
	case HQ10SCANCEL_PAD:	return 3;
	case HQ10TCANCEL_PAD:	return 4;
	case HQ10WTRANK_PAD:	return 5;
	case HQ10HYCJ_PAD:		return 6;
	}
	return 0;
}
...
DWORD CComView::GetAssis1FromSwitch(int nWhich)
{
	...
	case 6:
		return HQCDP_PAD;
	case 7:
		return (m_nMainPad==ZST_PAD)?BIGVOL_PAD:CB_PAD;
	case 8:
		return WEB_PAD;
	}
	return 0;
...
{
	return (AssisPad1==CB_PAD||AssisPad1==BIGVOL_PAD||AssisPad1==WEB_PAD);
}
...
//	m_pSwitchBar1->SetStyle(SwitchStyle_samebutton);
	m_pSwitchBar1->SetStyle(SwitchStyle_switchbutton);
	if(!m_pSwitchBar1 || !m_pSwitchBar1->Create(_T("NewZfjsTab"),"",WS_CHILD|WS_VISIBLE,CRect(0,0,0,0),this,ID_COMSWITCHBAR1))
		return -1;
	for(i=j=0;i<SwitchUnitNum;i++)
	{
		if(i==2||i==3) continue;
		m_pSwitchBar1->InsertSwitchLabel(j++,_F(SwitchUnitStr[i]),_F(SwitchUnitStr_Tips[i]),WM_COMMAND,ID_CORNER_1+i);
	}
	//创建Assis2区切换按钮
...
void CComView::OnInitialUpdate()
{
	...
	theViewManager.AddView("组合图", this);	
	ConstructPad(TRUE);
}
...
BOOL CComView::Proc_Special_MessageCode(int tmpMessageCode)
{
	...
	if(tmpMessageCode == 900)
	{
		...
		if(g_nMul_CurGP < 0)
			...
			SetPageGP(indexbuffer,totalnum,g_nMul_CurGP);
		m_nCurMulGp = 0;
...
void CComView::CreateUnit(int which,int initGPNo,int type,int style)
{
	...
	case CB_UNIT:	//移动筹码组件
		...
		break;
	case BIGVOL_UNIT://主力深度监控组件
		m_Units.pUnit[which] = new UBigVol(&ComInfo);
		break;
	case WEB_UNIT:	//通用浏览组件
		m_Units.pUnit[which] = new UWeb2(&ComInfo);
		break;
			...
			case MORE_GPPAGE:	//个股综合资讯
				{
					...
					CString tmpURL;
					tmpURL.Format("%swebs\\gppage_%d.htm",HomePath,GetExternWhichUnit(ComInfo.nWhichUnit,ComInfo.nInCom2));
...
void CComView::ConstructPad(BOOL bFirst)
{
	...
	m_bShowTopView = m_bShowAssisPad2&&g_bTopViewStatus;
	int count = 0,i;
	switch(m_nMainPad)
	{
	case TICK_PAD:
	...
	case FXT_PAD:
		...
		case BIGVOL_PAD:
			...
			break;
		case WEB_PAD:
			CreateUnit(count++,m_nCurGP,WEB_UNIT);
			if(m_Units.pUnit[count-1])
			{
				UWeb2 *pUWeb = (UWeb2*)m_Units.pUnit[count-1];
				pUWeb->SetURL(g_NewJyCfg.RelativeInfo,FALSE,TRUE);
			}
			break;
		}
		switch(m_nAssisPad2)
...
void CComView::GetTVArea(int nFixWid,CRect &rc)
{
	if(!g_GH.bLeftHQInfo)
	{
		rc.left=max(0,m_Rect.Width()-nFixWid);
		rc.right=m_Rect.Width();
	}
	else
	{
	...
	rc.top=0;
	rc.bottom=m_Rect.Height();
}

void CComView::OnSwitchTopView(WPARAM wParam,LPARAM lParam)
{
	g_bTopViewStatus=!g_bTopViewStatus;
	if(g_bTopViewStatus)
		m_bShowAssisPad2=TRUE;
	m_bShowTopView = m_bShowAssisPad2&&g_bTopViewStatus;
...
void CComView::ResizePad(int flag,int nNewGP)
{
	...
	CRect	rc,rc2,DComRc,BSRc;
	int		UnitNo;
	BOOL	bShowBSUnit = FALSE;
	...
	case FXT_PAD:
		if(m_Units.nStatus[1] == U_SHOW)
		{
			if(m_bShowTopView)
			{
				...
				//再请求主体区
				GetMainArea(g_nFixedWidth+g_nFixedWidth2,rc,DComRc,BSRc,bShowBSUnit);
				SetUnitReadAndPaint(0,rc,flag);
				...
				SetUnitReadAndPaint(UnitNo++,rc,flag);
				//请求TV的数据
				GetTVArea(g_nFixedWidth+g_nFixedWidth2,rc);
				SetUnitReadAndPaint(UnitNo++,rc,flag);
				//分时图下面的UNIT
				SetUnitReadAndPaint(UnitNo++,BSRc,flag);
				//请求DCOM的数据
			...
			else
			{
				...
				if(m_bShowAssisPad2)
					...
					GetMainArea(g_nFixedWidth,rc,DComRc,BSRc,bShowBSUnit);
				SetUnitReadAndPaint(0,rc,flag);
		...
		else	//如果右边被隐藏
		{
			...
			GetMainArea(0,rc,DComRc,BSRc,bShowBSUnit);
			SetUnitReadAndPaint(0,rc,flag);
...
void CComView::SetSwitchPos()
{
	...
	if((m_nMainPad!=ZST_PAD&&m_nMainPad!=FXT_PAD) || m_Units.nStatus[1]==U_HIDE || rc.Height()<FIXED_LOWHEIGHT || m_bShowTopView) 
	{
	...
	if(m_bShowAssisPad2)
	{
		...
		m_pSwitchBar2->SetWindowPos(NULL,0,0,0,0,SWP_SHOWWINDOW);
//	if(m_nMainPad==ZST_PAD)
//		m_pSwitchBar1->ModifySwitchLabel(7-2,_F("主"),_F("主力监控"));
//	else if(m_nMainPad==FXT_PAD)
//		m_pSwitchBar1->ModifySwitchLabel(7-2,_F("筹"),_F("移动筹码分布"));
	if(m_pSwitchBar1) m_pSwitchBar1->SetCurLabel(m_nCurSwitch1);
	if(m_pSwitchBar2) m_pSwitchBar2->SetCurLabel(m_nCurSwitch2);
...
void CComView::SwitchPad(DWORD tmpNowPad,BOOL bLowHQPart,BOOL bProperty)
{
		 ...
		 ((m_nMainPad==ZST_PAD&&uLastMain==FXT_PAD)||(m_nMainPad==FXT_PAD&&uLastMain==ZST_PAD)))
	{
		...
		if(m_nDComUnit>1)
			SetUnitReadAndPaint(m_nDComUnit-1,BSRc,COM_ReadDraw);
		if(m_nDComCurGP>=0 && m_nDComUnit>=0)
	...
	else if(m_nAssisPad1!=uLastAssis1 || bLowHQPart)
	{
		...
		case BIGVOL_PAD:
			...
			break;	
		case WEB_PAD:
			CreateUnit(2,m_nCurGP,WEB_UNIT);
			if(m_Units.pUnit[2])
			{
				UWeb2 *pUWeb = (UWeb2*)m_Units.pUnit[2];
				pUWeb->SetURL(g_NewJyCfg.RelativeInfo,FALSE,TRUE);
			}
			break;	
		}
...
DWORD CComView::GetPadFromMessageCode(int MessageCode,const char *Code,short setcode)
{
	...
	case 507: //买卖力道(涨跌率)
	case 508:
		g_bTickTStatus=(MessageCode==508);
	...
	case 900: tmpPad = MULZST_PAD; break;
	case 901: 
	...
	case 902: 
		...
		break;
	case 903:
		if((m_nNowPad&0x00FF00)==WEB_PAD)										//如果已经是网页组件
			tmpPad = (m_nNowPad&0xFF0000)+HQTICK_PAD+m_nAssisPad2;
		else
			tmpPad = (m_nNowPad&0xFF0000)+WEB_PAD+m_nAssisPad2;
		break;
	}
	BOOL bProperty=IsLevel2Property((*g_pStockIO)[m_nCurGP]);
	...
	case VK_ADD:
		{
			...
			{
				m_nCurSwitch1--;
				if(m_nCurSwitch1<0) m_nCurSwitch1 = SwitchUnitNum-1;
			}
			else
				m_nCurSwitch1=(m_nCurSwitch1+1)%SwitchUnitNum;
...
void CComView::OnTimer(UINT nIDEvent) 
{
	...
	if(nIDEvent==0x1111||nIDEvent==0x1112||nIDEvent==0x1113||nIDEvent==0x1114)	
	{	//各组件的定时器
		if(g_bDowning)	return;
		int i;
		FOR_UNIT(i)
...
{
	if(m_bShowTopView && m_bShowAssisPad2)
		return FALSE;
...
void CComView::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	{
		CRect Corner1(m_Rect.Width()-20,0,m_Rect.Width(),20);
		CRect Corner2(0,0,0,0);
		if(!g_GH.bLeftHQInfo)
		{
			if(m_bShowAssisPad2)
				Corner1.SetRect(m_Rect.Width()-g_nFixedWidth-g_nFixedWidth2-20,0,m_Rect.Width()-g_nFixedWidth-g_nFixedWidth2,20);
			else
				Corner1.SetRect(m_Rect.Width()-g_nFixedWidth-20,0,m_Rect.Width()-g_nFixedWidth,20);
			Corner2.SetRect(m_Rect.left,0,m_Rect.left+20,20);
		}
...
void CComView::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if(m_bSplitting2)
	{
		...
		g_nFixedWidth2 = min(400,g_nFixedWidth2);
		ResizePad();
		SetCursor(m_hVerSplit);
		return;
	}
...
{
	int i = HitTest(point);
	if( (i==2&&m_nMainPad!=MULZST_PAD&&m_nMainPad!=MULFXT_PAD&&FALSE) || 
	...
	if(m_nMainPad==MULFXT_PAD)
	{
			...
			||(pMsg->message == WM_CHAR && (pMsg->wParam == ']'||pMsg->wParam == '[')))
		{
			...
			else
			{
				...
				{
					if(m_Units.pUnit[i])
						((UFxt*)m_Units.pUnit[i])->ProcessMsg(WM_KEYDOWN,VK_F8);
				}
			}
			return TRUE;
		}
...
void CComView::OnSwitchWeb()
{
	PostMessage(WM_KEYGUY_MESSAGE,903);
}
