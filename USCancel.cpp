...
USCancel::USCancel(LPCOMINFO pComInfo):UBase(pComInfo)
{
	m_bDataReady = FALSE;
	m_bEmpty = TRUE;
	m_nZoomFlag = 0;
...
int  USCancel::ProcessMsg(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		{
			...
			if(i>=0)
				tmpInfo=g_pStockIO->Get_MemStkInfo(g_Mtl.SingleCancelRankings[i].Code,m_setcode);
			else if(j>=0)
				tmpInfo=g_pStockIO->Get_MemStkInfo(g_Mtl.SingleCancelRankings[10+j].Code,m_setcode);
			if(tmpInfo)
...
void USCancel::ReadData(BOOL bDataReady,BOOL bReqData)
{
	if(!OnLine || !CanLevel2())
		return;
	if(!bDataReady)
	{
		if(bReqData)
		{
			...
			g_pGetData->GetSCancelRank(m_setcode,m_fltype);
		}
	}
	else
		g_pGetData->GetSCancelRankACK(g_Mtl.SingleCancelRankings);
}

void USCancel::DrawPart1(CDC *pDC,int TotalHeight,int TextH,int XWidth,int XFrom[],int YFrom)
{
	if(TotalHeight<2*TextH) return;
	g_d.DisplayText(pDC,XFrom[0],YFrom,VipColor.TXTCOLOR,_F("委卖撤单"));
	g_d.DisplayText(pDC,XFrom[1],YFrom,VipColor.TXTCOLOR,XWidth,_F("撤单量"));
	g_d.DisplayText(pDC,XFrom[2],YFrom,VipColor.TXTCOLOR,XWidth,_F("委托价"));	
	YFrom+=TextH;
	...
	for(i=0;i<min(10,row);i++)
	{
		...
		if(tmpInfo)
		{
			...
			g_d.DisplayText(pDC,XFrom[1],YFrom,VipColor.VOLCOLOR,XWidth,"%.0f",g_Mtl.SingleCancelRankings[i].Quantity/tmpInfo->Unit);
			g_d.BjPrintf(pDC,XFrom[2],YFrom,g_Mtl.SingleCancelRankings[i].Price,tmpInfo->Close,m_XsFlag,XWidth);
			YFrom+=TextH;
		}		
	}
}

void USCancel::DrawPart2(CDC *pDC,int TotalHeight,int TextH,int XWidth,int XFrom[],int YFrom)
{
	if(TotalHeight<2*TextH) return;
	...
	for(i=0;i<min(10,row);i++)
	{
		tmpInfo=g_pStockIO->Get_MemStkInfo(g_Mtl.SingleCancelRankings[10+i].Code,m_setcode);
		if(tmpInfo)
		{
			size=g_d.DisplaySameWidtchText(pDC,XFrom[0],YFrom,GetBlockGPColor(tmpInfo,GridColor.Grid_CodeName_Color),tmpInfo->Name);
			m_BuyRect[i].SetRect(XFrom[0],YFrom,XFrom[0]+size.cx,YFrom+TextH-2);
			g_d.DisplayText(pDC,XFrom[1],YFrom,VipColor.VOLCOLOR,XWidth,"%.0f",g_Mtl.SingleCancelRankings[10+i].Quantity/tmpInfo->Unit);
			g_d.BjPrintf(pDC,XFrom[2],YFrom,g_Mtl.SingleCancelRankings[10+i].Price,tmpInfo->Close,m_XsFlag,XWidth);
			YFrom+=TextH;
		}		
	}
}

void USCancel::DrawIt(CDC *pDC)
{
