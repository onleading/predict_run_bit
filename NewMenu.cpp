...
{
	// ( 86a + 14b ) / 100
	return g_GUI.MixedColor(ColA,86,ColB);
}

COLORREF MidColor(COLORREF ColA,COLORREF ColB)
{ 
	// (7a + 3b)/10
	return g_GUI.MixedColor(ColA,70,ColB);
}
...
int CNewMenuIcons::AddGloomIcon(HICON hIcon, int nIndex)
{
	...
	for(int i=0;i<size.cx;++i)
	{
		for(int j=0;j<size.cy;++j)
		{
			...
			myDC.SetPixel(i,j,g_GUI.DarkenScaleColor(50,crPixel));
		}
	}
...
int CNewMenuIcons::AddGrayIcon(HICON hIcon, int nIndex)
{
	...
	for(int i=0;i<size.cx;++i)
	{
		for(int j=0;j<size.cy;++j)
		{
			...
			myDC.SetPixel(i,j,MixedColor(g_GUI.LightenScaleColor(100,g_GUI.GrayColor(crPixel)),crMenu));      
		}
	}
	myDC.SelectObject(pOldBitmap);
	if(nIndex==-1)
...
const Win32Type g_Shell = g_GUI.IsShellType();
...
m_pData(NULL)
{
	...
	m_bitmapBackground=RGB(192,192,192);//GetSysColor(COLOR_BTNFACE);
}
...
void CNewMenu::MeasureItem( LPMEASUREITEMSTRUCT lpMIS )
{
	...
	CNewMenuItemData *pdate=(CNewMenuItemData*)(lpMIS->itemData);
	CString temp2= pdate->GetString();

	BOOL bIsMenuBar = IsMenuBar(UIntToHMenu(lpMIS->itemID));
	BOOL bIsMenu = bIsMenuBar;

	if(/*temp2.Find("权息")!=-1 && */bIsMenuBar)
	{
		MENUITEMINFO mii;
		mii.cbSize = sizeof(mii);
		mii.fMask = MIIM_SUBMENU;
		if(GetMenuItemInfo(lpMIS->itemID, &mii) && mii.hSubMenu==NULL)
		{
			bIsMenuBar = FALSE;
			bIsMenu = FALSE;
		}
	...
	if(!bIsMenuBar && m_hParentMenu && bIsMenu)//::IsMenu(UIntToHMenu(lpMIS->itemID)) )
	{
...
void CNewMenu::DrawMenuTitle(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
	...
	if(pMenuData->m_nTitleFlags&MFT_GRADIENT)
	{
		if(pMenuData->m_nTitleFlags&MFT_SIDE_TITLE)
		{
			g_GUI.DrawGradient(pDC,rect,colorMenu,::GetSysColor(COLOR_ACTIVECAPTION),false);
		}
		else
		{
			g_GUI.DrawGradient(pDC,rect,::GetSysColor(COLOR_ACTIVECAPTION),colorMenu,true);
		}
	}
	else
	{
...
void CNewMenu::DrawItem_WinXP(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
	...
	COLORREF colorMenu = RGB(243,242,240);
	COLORREF colorBitmap = RGB(219,216,210);
	COLORREF colorSel = RGB(173,185,216);
	...
	if(bIsMenuBar && (lpDIS->itemState&ODS_SELECTED) )
	{
		if(!(lpDIS->itemState&ODS_INACTIVE) )
		{
			...
			{ 
				...
				Pen.DeleteObject();
				COLORREF clrPen = g_GUI.DarkenScaleColor(128,GetMenuBarColor());
				...
				{ 
					...
					for (Y=4; Y<8 ;Y++)
					{
						pDC->SetPixel(rect.right-X,Y+rect.top,g_GUI.DarkenScaleColor(2* 3 * X * (Y - 3), ColB)) ;
					}
					for (Y=8; Y<=(winH-1) ;Y++)
					{
						pDC->SetPixel(rect.right - X, Y+rect.top, g_GUI.DarkenScaleColor(2*15 * X, ColB) );
					}
				}
			}
		}
	...
	if( state & MF_SEPARATOR )
	{
		...
		else
		{
			...
			pDC->FillSolidRect(rect,RGB(161,172,193));
		}
	}
	else
	{
		...
		if(!strText.IsEmpty())
		{
			...
			if(tablocr!=-1)
			{
				...
				leftStr=strText.Left(/*strText.Find(_T('\t'))*/tablocr);
			}
			else 
				leftStr=strText;
				...
				(bIsMenuBar && lpDIS->itemState&ODS_INACTIVE) )
			{
				...
				OldTextColor = pDC->SetTextColor(RGB(166,166,166));
			}
			else
			{
				// Draw the text normal
				OldTextColor = pDC->SetTextColor(GetSysColor(COLOR_MENUTEXT));
				for(int m=0;m<g_nSpecMenuColor;m++)
				{
					if(lpDIS->itemID==g_SpecMenuColor[m].ItemID || (g_SpecMenuColor[m].ItemString[0]!=0 && strcmp(g_SpecMenuColor[m].ItemString,(char*)(LPCSTR)leftStr)==0))
					{
						OldTextColor = pDC->SetTextColor(g_SpecMenuColor[m].Color);
						break;
					}
				}
			}
		...
		if(!bIsMenuBar)
		{
			...
			if(checkflag||standardflag||selectedflag||disableflag)
			{
				...
				else
				{
					...
					if(state & ODS_DISABLED)
					{
						...
						{
							CBrush Brush;
							Brush.CreateSolidBrush(pDC->GetNearestColor(g_GUI.DarkenScaleColor(70,colorBitmap)));
					...
					else
					{
						...
						{ 
							if(m_bEnableXpBlendig)
							{
								...
								pDC->DrawState(ptImage, size, hDrawIcon2, DSS_NORMAL|DST_ICON,(HBRUSH)NULL);
								DestroyIcon(hDrawIcon2);
							}
							else
							{
			...
			if(pMenuData->m_nMenuIconOffset<0 && state&ODS_CHECKED && !checkflag)
			{
				...
				if(state&ODS_CHECKED || info.hbmpUnchecked) 
				{
					...
					{
						pDC->FillSolidRect(rect,RGB(173,185,216));
						//          pDC->FillSolidRect(rect,MixedColor(colorBitmap,GetSysColor(COLOR_HIGHLIGHT)));
					}
					CPen* pOldPen = pDC->SelectObject(&Pen);
					CBrush* pOldBrush = (CBrush*)pDC->SelectStockObject(HOLLOW_BRUSH);
		...
		if(!strText.IsEmpty())
		{
			...
			if(!(lpDIS->itemState & ODS_GRAYED))
			{
				...
				for(int m=0;m<g_nSpecMenuColor;m++)
				{
					if(lpDIS->itemID==g_SpecMenuColor[m].ItemID || (g_SpecMenuColor[m].ItemString[0]!=0 && strcmp(g_SpecMenuColor[m].ItemString,(char*)(LPCSTR)leftStr)==0))
					{
						pDC->SetTextColor(g_SpecMenuColor[m].Color);
						break;
					}
				}
...
BOOL CNewMenu::IsMenuBar(HMENU hMenu)
{
	...
	if(bIsMenuBar)
	{
		...
		CNewMenu* pMenu = DYNAMIC_DOWNCAST(CNewMenu,
			CMenu::FromHandlePermanent(m_hParentMenu));
		if (pMenu!=NULL)
		{
...
void CNewMenu::MeasureItem_WinXP( LPMEASUREITEMSTRUCT lpMIS, BOOL bIsMenuBar )
{
	...
	else
	{
		...
		{
			//当菜单栏上有非popup项时,也要按照pupup菜单计算宽度
			if(!m_hParentMenu && !IsPopup())
				lpMIS->itemWidth = size.cx;	
			else 
				lpMIS->itemWidth = m_iconX + size.cx + m_iconX + GAP;
		}
		...
		lpMIS->itemHeight = temp>m_iconY+5 ? temp : m_iconY+5;
...
BOOL CNewMenu::ModifyODMenu(LPCTSTR lpstrText, UINT nID, int nIconNormal)
{
	...
	return (CMenu::ModifyMenu(nID,MF_BYCOMMAND,nID,(LPCTSTR)(pItemData->m_szMenuText)));
}
					   ...
					   m_pDrawTitle(pDrawTitle)
{
	...
	if(!IsShadowEnabled() && m_dwThemeId!=CNewMenu::STYLE_XP)
	{
...
{ 
	...
	{ 
		...
		for (Y=4; Y<8 ;Y++)
		{
			...
			SetPixel(hDC,winW-X,Y,g_GUI.DarkenScaleColor(2* 3 * X * (Y - 3), c)) ;
		}
		for (Y=8; Y<=(winH-5) ;Y++)
		{
			COLORREF c = GetPixel(hDcDsk, xOrg + winW - X, yOrg + Y) ;
			SetPixel( hDC, winW - X, Y, g_GUI.DarkenScaleColor(2* 15 * X, c) );
		}
		for (Y=(winH-4); Y<=(winH-1) ;Y++)
		{
			COLORREF c = GetPixel(hDcDsk, xOrg + winW - X, yOrg + Y) ;
			SetPixel( hDC, winW - X, Y, g_GUI.DarkenScaleColor(2* 3 * X * -(Y - winH), c)) ;
		}
	} 
	...
	for(Y=1; Y<=4 ;Y++)
	{
		...
		for(X=4; X<=7 ;X++)
		{
			...
			SetPixel( hDC, X, winH - Y, g_GUI.DarkenScaleColor(2*3 * (X - 3) * Y, c)) ;
		}
		for(X=8; X<=(winW-5) ;X++)
		{
			COLORREF  c = GetPixel(hDcDsk, xOrg + X, yOrg + winH - Y); 
			SetPixel( hDC, X, winH - Y, g_GUI.DarkenScaleColor(2* 15 * Y, c)) ;
		}
	}   
...
BOOL CMenuTheme::OnCalcFrameRect(HWND hWnd,LPRECT pRect)
{
	if(GetWindowRect(hWnd,pRect))
	{
		if(DoDrawBorder())
		{
			pRect->top += 2;
			pRect->left += 2;
			if(m_dwThemeId==CNewMenu::STYLE_XP)
			{
				pRect->bottom -= 2;
				pRect->right -= 2;
			}
...
BOOL CMenuThemeXP::OnDrawBorder(HWND hWnd, HDC hDC)
{
	...
	if(DoDrawBorder() && pData)
	{
		...
		COLORREF clrBorder = g_GUI.DarkenScaleColor(128,crMenuBar);
		COLORREF colorBitmap = MixedColor(crMenuBar,crWindow);
		
		if(!IsShadowEnabled())
		{
//			pDC->Draw3dRect(CRect(CPoint(1,1),CSize(winW-2,winH-2)),RGB(255,0,0),RGB(255,0,0));
//			pDC->Draw3dRect(CRect(CPoint(0,0),CSize(winW,winH)),RGB(0,255,0),RGB(0,255,0));
			pDC->Draw3dRect(CRect(CPoint(1,1),CSize(winW-2,winH-2)),crThinBorder,crThinBorder);
			pDC->Draw3dRect(CRect(CPoint(0,0),CSize(winW,winH)),clrBorder,clrBorder);
...
BOOL CMenuThemeXP::OnEraseBkgnd(HWND hWnd, HDC hDC)
{
	...
	pDC->FillSolidRect (Rect,CNewMenu::GetMenuBarColor());
//	pDC->FillSolidRect(Rect,RGB(0,0,1));
	return TRUE;
}
