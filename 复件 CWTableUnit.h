...
public:
	...
	short	GetTimeParaFlag()
	{
		if(strlen(m_Cfg.SQLstatement)<1) return 2;
		BOOL bTime1 = strstr(m_Cfg.SQLstatement, "#time1")!=NULL;
		BOOL bTime2 = strstr(m_Cfg.SQLstatement, "#time2")!=NULL;
		return bTime1+bTime2;
	}

	void	GetTimeParam(long &nTime1, long &nTime2)
	{
		nTime1 = m_CwParam.nCwTimeS;
		nTime2 = m_CwParam.nCwTimeE;
	}

	CString GetTitleString() {return m_strTitle;}
...
private:
	...
	//时间统计
	long			m_nInitPassedTime, m_nInsertPassedTime, m_nShowPassedTime,m_nOtherPassedTime;
};
