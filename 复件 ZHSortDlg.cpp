...
void MakeZHSortTitle(char (*TypeStr)[25])
{
	...
	strcpy(TypeStr[0],"今日涨幅排名");
	strcpy(TypeStr[1],"今日跌幅排名");
	strcpy(TypeStr[2],"今日振幅排名");
	tempStr.Format("%d分钟涨速排名",g_nZFFzNum);
	strcpy(TypeStr[3],tempStr);
	tempStr.Format("%d分钟跌速排名",g_nZFFzNum);
	strcpy(TypeStr[4],tempStr);
	strcpy(TypeStr[5],"今日量比排名");
	if(!g_bSupportDDE)
	{
		strcpy(TypeStr[6],"今日委比前排名");
		strcpy(TypeStr[7],"今日委比后排名");
	}
	else
	{
